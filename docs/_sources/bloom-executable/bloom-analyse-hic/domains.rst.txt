...
.. note::
    ...
    The following examples use the matrix files in bloom format. If you want to try the same
    commands using Juicer ``.hic`` files, replace ``output/hic/binned/bloom_example_100kb.hic``
    with ``architecture/other-hic/bloom_example.juicer.hic@100kb``. You will also need to adjust the
    `-vmax` value in the triangular matrix plot to `50`. If you want to work with
    Cooler files in this tutorial, use ``architecture/other-hic/bloom_example.mcool@100kb``.
...
.. image:: images/bloom_example_100kb_tads.png


bloom provides multiple "scores" that are designed to find the boundaries between domains.
...
.. image:: images/bloom_example_100kb_tads_insulation_score_example.png
...
Use ``bloom insulation`` to calculate the insulation score from the command line:
...
``bloom insulation`` is typically used to calculate insulation scores with multiple window
...
We can easily plot all insulation scores at the same time using ``bloomplot``:
...
.. image:: images/bloom_example_50kb_tads_insulation.png
...
The default output format has maximum compatibility within bloom, but other tools like
...
.. code::
    ...
    architecture/domains/bloom_example_100kb.insulation_1.5mb.bed
    architecture/domains/bloom_example_100kb.insulation_1mb.bed
    architecture/domains/bloom_example_100kb.insulation_2.5mb.bed
    architecture/domains/bloom_example_100kb.insulation_2mb.bed
    architecture/domains/bloom_example_100kb.insulation_3.5mb.bed
    architecture/domains/bloom_example_100kb.insulation_3mb.bed
    architecture/domains/bloom_example_100kb.insulation_4mb.bed
...
previously calculated scores object, simply run ``bloom insulation`` without any
parameters:
...
You can plot scores from one or more window sizes using the ``line`` plot in ``bloomplot``:
...
.. image:: images/bloom_example_50kb_tads_insulation_1mb.png
...
By default, ``bloom insulation`` will normalise the insulation scores to the chromosomal
...
Normally, ``bloom insulation`` will use the arythmetic mean of the chromosomal scores to
...
50% of unmappable regions, the score will be NaN. ``bloom insulation`` offers the option to
...
.. image:: images/bloom_example_50kb_tads_insulation_imputed.png
...
between two self-interacting domains, or TADs. You can use ``bloom boundaries`` to identify these
regions:
...
When we run ``bloom boundaries`` on the above example using 1mb and 2mb as the window sizes:
...
.. code::
    ...
    bloom_example_100kb.insulation_boundaries_1mb.bed
    bloom_example_100kb.insulation_boundaries_2mb.bed
...
.. image:: images/bloom_example_50kb_tads_insulation_1mb_boundaries.png

As you can see, lower minima get higher scores. By default, ``bloom boundaries`` outputs all
...
By default, ``bloom boundaries`` will return minima as matrix bins. However, since the boundary
...
Use ``bloom directionality`` to calculate the directionality index from the command line:
...
``bloom directionality`` is very similar in syntax to ``bloom insulation``.
...
We can easily plot all directionality indexes at the same time using ``bloomplot``:
...
.. image:: images/bloom_example_50kb_tads_directionality.png

To export the directionality index to other genomic formats using ``bloom directionality``
follow the instructions as for ``bloom insulation``.
...
bloom, and refer the user to one of the many available tools for TAD calling that offer
