...
.. note::
    ...
    with ``architecture/other-hic/bloom_example.juicer.hic@100kb``. If you want to work with
...
You can easily create your own aggregate plots using ``bloom aggregate``.
...
You can provide ``bloom aggregate`` with a list of genomic regions in any of the common
...
By default, if you provide ``bloom aggregate`` with a list of regions, it will extract
...
2014 <http://dx.doi.org/10.1016/j.cell.2014.11.021>`_). ``bloom aggregate`` will ignore
...
This command only produces an AggregateMatrix file (``bloom_example_100kb.agg``), which
is useful for further usage with bloom, but not easily readable. To extract the aggregate
...
.. image:: images/bloom_example_100kb.agg.png
...
.. image:: images/bloom_example_100kb_oe.agg.png
...
.. image:: images/bloom_example_100kb_oe_large.agg.png
...
.. image:: images/bloom_example_100kb_oe_large_res.png
...
aggregate analysis, such as TAD boundaries. ``bloom aggregate`` provides the ``-w``
...
.. image:: images/bloom_example_100kb_boundaries.agg.png
...
.. image:: images/bloom_example_100kb_boundaries_oe.agg.png
...
``bloom aggregate`` to make aggregate loop plots. The preset for this is ``--loops``.
