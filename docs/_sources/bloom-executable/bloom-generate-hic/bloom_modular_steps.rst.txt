...
=============================
bloom map: Mapping FASTQ files
=============================

To (iteratively) map FASTQ files directly with bloom, use the ``bloom map`` command.

Here is a minimal example:

.. code:: bash

    bloom map SRR4271982_chr18_19_1.fastq.gzip bwa-index/hg19_chr18_19.fa \
...
and ``bloom map`` will automatically convert the mapping output to BAM format.

Importantly, ``bloom map`` will autodetect if you supply a BWA or Bowtie2 index, so
the following command would use Bowtie2 as a mapper:

.. code:: bash

    bloom map SRR4271982_chr18_19_1.fastq.gzip hg19_chr18_19/hg19_chr18_19 \
             SRR4271982_chr18_19_1.sam
...
``bloom auto`` parallelises mapping by spawning multiple mapping processes internally.
...
By default, ``bloom auto`` performs iterative mapping: Reads are initially trimmed to 25bp
...
===============================================
bloom pairs: Generating and filtering read Pairs
===============================================

The ``bloom pairs`` command handles the creation and modification of Pairs objects, which represent
...
or an existing bloom Pairs object. For all but the latter, an output file is also required as last
positional argument, for Pairs files the output file is optional. If omitted, filtering is performed
in place.

.. warning::

    If you are providing sorted SAM/BAM files as input to ``bloom pairs``, they must be sorted either
    using ``samtools sort -n`` or ``sambamba sort -N``. You cannot use ``sambamba sort -n``
    (note the lowercase ``n``), as this will do a lexicographic sort, which is incompatible with bloom!
...
.. code:: bash
    ...
    bloom pairs output/sam/SRR4271982_chr18_19_1_sort.bam \
...
.. code:: bash
    ...
    bloom pairs 4d_nucleome.pairs output/pairs/4d_nucleome.pairs \
               -g hg19_chr18_19_re_fragments.bed
...
.. code:: bash
    ...
    bloom pairs hic_pro.validPairs output/pairs/hic_pro.pairs \
               -g hg19_chr18_19_re_fragments.bed

Existing bloom Pairs object:

.. code:: bash

    bloom pairs output/pairs/SRR4271982_chr18_19.pairs
...
.. note::
   ...
   **Read names of the two mates must be identical**. Some read names, especially those obtained
   ...
   to the read name. This interferes with the sorting and read pairing step in bloom, and you
...
``bloom pairs`` provides a lot of parameters for filtering read pairs according to different
...
.. code::
    ...
    bloom pairs --re-dist-plot re-dist.png output/pairs/SRR4271982_chr18_19.pairs
...
.. code::
    ...
    bloom pairs --ligation-error-plot ligation-err.png output/pairs/SRR4271982_chr18_19.pairs


.. image:: images/ligation-err.png

Otherwise 1-10kb are often a reasonable cutoffs. You can also let bloom attempt to find suitable
...
========================================================
bloom hic: Generating, binning, and filtering Hic objects
========================================================

The ``bloom hic`` command is used to generate fragment-level and binned Hi-C matrices.

You can use bloom Pairs files as input for ``bloom hic``:

.. code:: bash

    bloom hic output/pairs/SRR4271982_chr18_19.pairs output/hic/fragment_level.hic
...
.. code:: bash
    ...
    bloom hic output/hic/fragment_level.hic output/hic/binned/example_1mb.hic -b 1mb
...
``bloom hic`` provides a few filtering options. Most likely you want to apply a coverage filter
...
.. note::
  ...
  The default normalisation mode is per chromosome, and the bloom analysis functions assume
