...
.. note::
    ...
    Before we start introducing the neat little interface functions that bloom provides for
    interacting with genomic regions, region pairs and matrix data, we want to briefly discuss
    the terminology used by bloom:

    Chromosome conformation capture data describes associations between pairs of genomic regions.
    In the literature, you will see them called "bins", or "loci", bloom generally uses the term
    ...
    and tools therefore use other terminology, such as "pixels". bloom uses a term from network
    ...
    connecting a pair of regions. Two regions in bloom are considered unconnected if their edge
    weight is 0. Instead of calling the regions connected by an edge "region1" and "region2" or
    something similar, they are called "source" and "sink" in bloom, again borrowing from network terminology.
    While the edge in Hi-C matrices has no directionality (we cannot say that region A interacts with
    region B but not vice versa), the convention in bloom is that the region index of the source
    region is smaller that that of the sink region.
    We believe this is a sufficiently neutral terminology for bloom data.


Being able to load different data types with the same command is the first convenient
feature of :func:`~bloom.__init__.load`. The second is that most objects returned by
:func:`~bloom.__init__.load` and supported by bloom in general share common interfaces
...
- :class:`~genomic_regions.RegionBased`: bloom builds heavily on the
  ...
  bloom files. Find out more in :ref:`genomic_regions`.

- :class:`~bloom.RegionPairsContainer`: This interface extends
  ...
  :func:`bloom.RegionpairsContainer.edges` function to iterate over all edges in an object,
  query only a subset, normalise edge weights on the fly, hide and reveal filtered
  edges, and more. This interface supports bloom files like :class:`~bloom.ReadPairs` and
  :class:`~bloom.Hic`, Cooler single and multi-resolution files and Juicer files.
  To find out everything about this interface, go to :ref:`edges_interface`.

- :class:`bloom.RegionMatrixContainer`: In many cases, such as Hi-C data, edges and their
  associated scores ("weights") can be represented as a matrix. This interface extends
  :class:`~bloom.RegionPairsContainer` with matrix-specific properties and functions,
  ...
  bloom files like :class:`~bloom.Hic` and  :class:`~bloom.ComparisonMatrix`, Cooler
