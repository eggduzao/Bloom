...
:class:`~bloom.matrix.RegionPairsContainer` (see previous section :ref:`edges_interface`),
...
After loading a dataset using :func:`~bloom.tools.load.load`, you can check for
support of the :class:`~bloom.matrix.RegionMatrixContainer` interface with:
...
The current list of bloom classes supporting :class:`~bloom.matrix.RegionMatrixContainer` is:
...
:func:`~bloom.matrix.RegionMatrix.matrix` function:
...
Of course, the :func:`~bloom.matrix.RegionMatrixContainer.matrix` function supports matrix subsets:
...
The returned object is of type :class:`~bloom.matrix.RegionMatrix`, which is a subclass
of Numpy's masked :class:`~numpy.ma.array` with added perks for genomic region handling.

A :class:`~bloom.matrix.RegionMatrix` can be used like any other numpy matrix,
...
are ignored for most downstream analysis to prevent artifacts. By default, bloom masks
...
:class:`~bloom.matrix.RegionMatrix` objects also keep track of the regions corresponding to
...
You can subset a :class:`~bloom.matrix.RegionMatrix` using indexes or region intervals:
...
:func:`~bloom.matrix.RegionMatrixContainer.matrix` supports all arguments also available for
:func:`~bloom.matrix.RegionPairsContainer.edges`, but it is not necessary to use lazy loading.
...
:func:`~bloom.matrix.RegionMatrixContainer.matrix`. Most notably, you can use the
...
:class:`~bloom.matrix.RegionMatrixContainer.expected_values` to calculate the expected
