...
After we have obtained a filtered :class:`~bloom.pairs.ReadPairs` object, we can easily
convert it into a :class:`~bloom.hic.Hic` matrix.
...
Hi-C matrix, using the same fragment definitions as in the original :class:`~bloom.pairs.ReadPairs`
object.
...
Just like :class:`~bloom.pairs.ReadPairs`, :class:`~bloom.hic.Hic` can also be filtered. Currently,
the two available filters are :class:`~bloom.hic.LowCoverageFilter` and
:class:`~bloom.edge.DiagonalFilter`:
...
- :class:`~bloom.hic.LowCoverageFilter` removes all Hi-C "edges" (pixels) in regions that have
...
- :class:`~bloom.edge.DiagonalFilter` removes all edges at the diagonal. Some researcher have
...
Knight-Ruiz matrix balancing (KR, :class:`~bloom.hic.kr_balancing`) or iterative correction (ICE,
:class:`~bloom.hic.ice_balancing`). KR balancing is typically faster, but also consumes a lot
