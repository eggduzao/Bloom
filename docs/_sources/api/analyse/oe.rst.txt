...
:class:`~bloom.matrix.RegionMatrixContainer` objects (see :ref:`here <matrix_interface>`) have a builtin
function to calculate expected values from existing matrix data called
:func:`~bloom.matrix.RegionMatrixContainer.expected_values`. This function calculates and returns
...
bloom also has a built-in function for plotting the expected values,
:func:`~bloom.plotting.distance_decay_plot`. Additional named arguments
...
Note: as Hi-C matrices are normalised on a per-chromosome basis in bloom by default, it would be misleading
...
Expected values rarely need to be calculated explicitly in bloom analysis functions, but will be calculated
