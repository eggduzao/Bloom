...
Also have a look at the command line documentation at :ref:`bloom-loops` for the
command-line approach to loop calling in bloom.

Loop calling in bloom is a 3-step process:
...
:class:`~bloom.peaks.RaoPeakCaller` and :func:`~bloom.peaks.RaoPeakCaller.call_peaks` function:
...
This creates a :class:`~bloom.peaks.RaoPeakInfo` object, which contains a bunch of information
...
:func:`~bloom.peaks.RaoPeakCaller.call_peaks` must then be called from a head node in order to be
...
To access the new pixel information, you can use the :func:`~bloom.peaks.RaoPeakInfo.peaks` iterator:
...
- :class:`~bloom.peaks.EnrichmentPeakFilter` filters pixels below a minimum O/E in specific
  neighborhoods
- :class:`~bloom.peaks.FdrPeakFilter` filters pixels with an FDR higher than specified in each
  neighborhood
- :class:`~bloom.peaks.MappabilityPeakFilter` filters pixels with a minimum mappable fraction
  below the specified cutoffs
- :class:`~bloom.peaks.ObservedPeakFilter` filters pixels that do not have a minimum number of
  raw reads mapping to them
- :class:`~bloom.peaks.DistancePeakFilter` filters pixels that are closer than a certain number
  of bins from the diagonal

Each of these is instantiated and then added to the object using
:func:`~bloom.peaks.RaoPeakInfo.filter`. When applying multiple filters it is recommended to
first queue them using ``queue=True`` and to run all of them together with
:func:`~bloom.peaks.RaoPeakInfo.run_queued_filters`:
...
using :class:`~bloom.peaks.RaoMergedPeakFilter`:
...
:func:`~bloom.peaks.PeakInfo.to_bedpe`:
