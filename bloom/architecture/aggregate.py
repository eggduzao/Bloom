...
class AggregateMatrix(FileGroup):
    ...
    def from_center(cls, matrix, regions, window=200000,
        ...
        .. code::
            ...
            import bloom
            hic = bloom.load("/path/to/matrix.hic")
            tad_boundaries = bloom.load("/path/to/tad_boundaries.bed")

            # run aggregate analysis
            am = bloom.AggregateMatrix.from_center(hic, tad_boundaries.regions,
                                                  window=500000)
    ...
    def from_regions(cls, hic, tad_regions, pixels=90,
        ...
        .. code::
            ...
            import bloom
            hic = bloom.load("/path/to/matrix.hic")
            tads = bloom.load("/path/to/tads.bed")

            # run aggregate analysis
            am = bloom.AggregateMatrix.from_regions(hic, tads.regions,
    ...
    def from_center_pairs(cls, hic, pair_regions, window=None, pixels=16,
        ...
        :param kwargs: Keyword arguments passed on to
                       :func:`~bloom.architecture.aggregate.extract_submatrices`
        :return: :class:`~bloom.architecture.aggregate.AggregateMatrix`
        """
        kwargs.setdefault('oe', True)
        kwargs.setdefault('keep_invalid', False)
        kwargs.setdefault('log', True)
