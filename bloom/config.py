...
4. .bloom.conf in home folder
5. ~/.config/bloom.conf
6. /etc/bloom/bloom.conf

All configuration files will be read, but the options of lower-ranking
locations will be overwritten.

To write a default configuration file use  :code:`bloom write-config`
...
.. code::
    ...
    write_default_config('~/.bloom.conf')  # or any other location

"""

from bloom.tools.general import Map
import yaml
import os
import copy
import warnings
...
_config_file_name = 'bloom.conf'
_environment_variable_name = 'BLOOM_CONF'
...
_config_file_locations = (
    ...
    os.path.join(os.path.expanduser("~"), '.config', 'bloom', _config_file_name),
    # /etc/bloom (MySQL style)
    os.path.join("/etc/bloom/", _config_file_name)
)
...
def read_file_configs(config_file_locations=None):
    """
    Read all bloom.conf configuration files found in
    1. current directory
    2. specified by environment variable bloom_CONF
    3. bloom.conf in home folder
    4. .bloom.conf in home folder
    5. ~/.config/bloom.conf
    6. /etc/bloom/bloom.conf
...
def write_default_config(file_name=None, overwrite=False):
    ...
    :param file_name: Output path, e.g. ~/.bloom.conf
    :param overwrite: Overwrite existing file if True. Default: False
    """
    if file_name is None:
        file_name = os.path.expanduser('~/.bloom.conf')
    else:
