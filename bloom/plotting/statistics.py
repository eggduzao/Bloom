...
def summary_statistics_plot(stats, ax=None, exclude=None, include=None, **kwargs):
    """
    Barplot with filter statistics for bloom objects.
...
def ligation_bias_plot(pairs, ax=None, log=False, **kwargs):
    ...
    :param pairs: Read pairs mapped to genomic regions (:class:`~bloom.pairs.ReadPairs`)
    :param ax: (optional) axis in which to plot the bars.
               Will use :code:`plt.gca()` if not specified
...
def restriction_site_distance_plot(pairs, ax=None, max_percentile=95,
    ...
    :class:`~bloom.pairs.ReDistanceFilter`, which excludes mate pairs with
    very large insert sizes.

    :param pairs: :class:`~bloom.pairs.ReadPairs`
...
def marginals_plot(matrix, chromosome, ax=None, lower=None, rel_cutoff=0.1, color='#3E896D',
    ...
    for the :class:`~bloom.hic.LowCoverageFilter`.

    :param matrix: :class:`~bloom.matrix.RegionMatrixContainer`
...
def pca_plot(pca_res, variance=None, eigenvectors=(0, 1),
    ...
    """
    Plot the results of a Hi-C PCA analysis from :func:`~bloom.architecture.comparisons.hic_pca`.

    :param pca_res: The PCA result from :func:`~bloom.architecture.comparisons.hic_pca`
    :param variance: A vector specifying the explained variance of each EV in the PCA or
                     the PCA object from :func:`~bloom.architecture.comparisons.hic_pca`.
