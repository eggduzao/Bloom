...
def get_region_based_object(input_object):
    if isinstance(input_object, RegionBased):
        return input_object
    return bloom.load(input_object)


def load_score_data(data):
    # If it's already an instance of bloom data, just return it
    if isinstance(data, RegionBased):
        return data

    # If it's anything else let's hope bloom.load can deal with it
    return bloom.load(data)
