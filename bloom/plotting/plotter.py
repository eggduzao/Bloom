...
from bloom import load
from genomic_regions import GenomicRegion, GenomicDataFrame, merge_overlapping_regions
from bloom.plotting.base_plotter import BasePlotter1D, ScalarDataPlot, BaseOverlayPlotter, \
                                       BasePlotter, BaseAnnotation
from bloom.plotting.hic_plotter import BasePlotterMatrix
from bloom.plotting.helpers import append_axes, style_ticks_whitegrid, get_region_field, \
...
import bloom
from bloom.tools.general import human_format
import logging
...
class GenomicFigure(object):
    ...
    def __init__(self, plots, width=4., ticks_last=False,
        ...
        :param list plots: List of plot instances each will form a separate panel in the figure.
                      Should inherit from :class:`~bloom.plotting.baseplotter.BasePlotter` or
                      :class:`~bloom.plotting.baseplotter.BaseAnnotation`
    ...
    def plot(self, region):
        ...
        :param region: A string describing a region "2L:10000000-12000000" or
                       a :class:`~bloom.data.genomic.GenomicRegion`.
        ...
        :type region: string or ~bloom.data.genomic.GenomicRegion
...
class RegionsValuesPlot(ScalarDataPlot):
    ...
    def __init__(self, regions, values, symmetry=None, **kwargs):
        ...
        :param aspect: Default aspect ratio of the plot. Can be overriden by setting
               the height_ratios in :class:`~bloom.plotting.GenomicFigure`
        """
...
class GenomicVectorArrayPlot(BasePlotterMatrix, BasePlotter1D):
    """
    Plot matrix from a :class:`~bloom.architecture.hic_architecture.MultiVectorArchitecturalRegionFeature` object.
    """

    def __init__(self, array, parameters=None, y_coords=None, y_scale='linear',
                 plot_kwargs=None, genomic_format=False, **kwargs):
        """
        :param array: :class:`~bloom.architecture.hic_architecture.MultiVectorArchitecturalRegionFeature`
...
class BigWigPlot(ScalarDataPlot):
    ...
    def _line_values(self, region):
        for i, b in enumerate(self.bigwigs):
            if isinstance(b, bloom.data.genomic.RegionBased):
...
class Virtual4CPlot(BasePlotter1D):
    ...
    This plot is an alternative to the :class:`~bloom.plotting.HicSlicePlot` with
    ...
    def __init__(self, hic, viewpoint, color='blue', alpha=1.0,
                 ...
                 *args, **kwargs):
        """
        :param hic: :class:`~bloom.Hic` or :class:`~bloom.RegionMatrix`.
        :param viewpoint: Viewpoint to use for virtual 4C. String, e.g. 
                "2L:1000000-1500000" or :class:`~bloom.GenomicRegion`.
        ...
        if isinstance(viewpoint, string_types):
            viewpoint = bloom.GenomicRegion.from_string(viewpoint)
