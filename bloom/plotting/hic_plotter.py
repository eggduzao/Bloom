...
from bloom.plotting.base_plotter import BasePlotterMatrix, BasePlotter1D, BasePlotter2D, ScalarDataPlot, \
                                       PlotMeta, BaseOverlayPlotter
from bloom.plotting.helpers import append_axes, style_ticks_whitegrid
...
def prepare_hic_buffer(hic_data, buffering_strategy="relative", buffering_arg=1,
    ...
    :param hic_data: :class:`~bloom.data.genomic.RegionMatrixTable` or
                     :class:`~bloom.data.genomic.RegionMatrix`
...
class BufferedMatrix(object):
    """
    Buffer contents of any :class:`~bloom.Hic` like objects. Matrix is
    ...
    def __init__(self, data, buffering_strategy="relative", buffering_arg=1,
        ...
        indexing using class:`~GenomicRegion` objects, such as class:`~bloom.Hic`
        or class:`~bloom.RegionMatrix` objects.
    ...
    def get_matrix(self, *regions):
        ...
        if self.smooth_sigma is not None:
            ...
            m = bloom.matrix.RegionMatrix(mf, row_regions=m.row_regions, col_regions=m.col_regions)
        return m
    ...
    **warning: should always be inherited from first, before the other
    ...
    def __init__(self, hic_data, adjust_range=False, buffering_strategy="relative",
        ...
        :param hic_data: Path to Hi-C data on disk or
                        :class:`~bloom.data.genomic.Hic` or :class:`~bloom.data.genomic.RegionMatrix`
        ...
        if isinstance(hic_data, string_types):
            hic_data = bloom.load(hic_data, mode="r")
...
class HicSlicePlot(ScalarDataPlot):
    ...
    def __init__(self, hic_data, slice_region, names=None,
        ...
        """
        :param hic_data: :class:`~bloom.Hic` or :class:`~bloom.RegionMatrix`. Can be list of
...
class TriangularMatrixPlot(BasePlotterHic, BasePlotter1D):
    ...
    def _mesh_data(self, region):
        ...
        hm_copy = bloom.matrix.RegionMatrix(np.copy(hm), col_regions=hm.col_regions,
