...
@deprecated(reason="Outdated API - use matrix copying instead! (bloom hic "
...
def convert_juicer_to_hic(juicer_file, genome_file, resolution, juicer_tools_jar_path=None,
    ...
    if juicer_tools_jar_path is None:
        raise ValueError("Just must provide the juicer tools jar path or set it in bloom.conf!")
...
class JuicerHic(RegionMatrixContainer):
    def __init__(self, hic_file, resolution=None, mode='r', tmpdir=None, norm=None):
        ...
        if tmpdir is None or (isinstance(tmpdir, bool) and not tmpdir):
            ...
            self.tmp_file_name = tmp_file_name(tmpdir, prefix='tmp_bloom', extension='_juicer.hic')
        ...
        if self.version > 8:
            warnings.warn(f"Support for Juicer .hic v{self.version} is still in beta. "
                          "Please report any issues to https://github.com/vaquerizaslab/bloom/issues/92")
        elif self.version > 9:
            raise ValueError(f"Juicer v{self.version} not currently supported. "
                             "Please report to https://github.com/vaquerizaslab/bloom/issues")
