...
class SgeTaskRunner(TaskRunner):
    def __init__(self, task_prefix=None,
        ...
        if task_prefix is None:
            ...
            self._task_prefix = 'bloom_' + random_name(6) + '_'
        else:
    ...
    def _submit_task(self, task, kill=True):
        ...
        with tempfile.NamedTemporaryFile('w', prefix='bloom_auto_', suffix='.sh') as tmp_file:
...
def auto_parser():
    ...
    parser.add_argument(
        ...
        nargs='+',
        help="Input files. "
             "bloom will try to guess the file "
    ...
    parser.add_argument(
        ...
        default='parallel',
        help="Choose how to run the commands in bloom auto. Options: "
             "'parallel' (default): Run bloom commands on local machine, "
             "use multiprocessing parallelisation. "
             "'sge': Submit bloom commands to a Sun/Oracle Grid Engine cluster. "
             "'slurm': Submit bloom commands to a Slurm cluster. "
             "'test': Do not run bloom commands but print all commands " 
    ...
    parser.add_argument(
        ...
        help="Path to a file with BASH commands that are executed "
             "after every bloom command that is run on a grid engine / cluster. "
...
def file_type(file_name):
    ...
    from bloom.pairs import HicProPairGenerator, FourDNucleomePairGenerator
    try:
...
def auto(argv, **kwargs):
    from bloom.tools.general import which
    ...
    if verbosity > 0:
        ...
        bloom_base_command = ['bloom', verbosity_flag]
    else:
        bloom_base_command = ['bloom']

    log_file = kwargs.get("log_file", None)
    if log_file is not None:
        bloom_base_command += ['-l', log_file]
    ...
    if run_with == 'parallel':
        ...
        if which(config.sge_qsub_path) is None:
            parser.error("Using SGE not possible: "
                         ...
                         "this path using bloom config files and the "
                         "'sge_qsub_path' parameter".format(config.sge_qsub_path))
        from bloom.tools.files import mkdir
    ...
    for i in range(len(file_types)):
        if file_types[i] not in ('fastq', 'sam', 'pairs_txt', 'pairs', 'hic'):
            import bloom
            try:
                ds = bloom.load(file_names[i], mode='r')
                if isinstance(ds, bloom.Hic):
                    file_types[i] = 'hic'
                elif isinstance(ds, bloom.ReadPairs):
                    file_types[i] = 'pairs'
                else:
                    raise ValueError("Could not detect file type using bloom load.")
            except ValueError:
    ...
    if len(fastq_files) > 0:
        ...
        for i, ix in enumerate(fastq_files):
            ...
            mapping_command = bloom_base_command + ['map', '-m', '25',
                                                   '-s', str(step_size),
                                                   '-t', str(threads)]
            ...
            if not mapper_parallel:
                mapping_command.append('--bloom-parallel')
            if split_fastq:
    ...
    if sam_sort:
        ...
        if len(sam_files) > 0:
            ...
            for i, ix in enumerate(sam_files):
                sort_command = bloom_base_command + ['sort_sam', '-t', str(sort_threads)]
                if use_sambamba:
    ...
    if len(pairs_txt_files) > 0:
        ...
        for ix in pairs_txt_files:
            ...
            pairs_command = bloom_base_command + ['pairs', '-f',
                                                 # loading
                                                 '-g', genome,
                                                 '-t', str(load_threads)]
    ...
    if len(pairs_files) > 0 and filter_pairs:
        ...
        for ix in pairs_files:
            ...
            pairs_command = bloom_base_command + ['pairs',
                                                 # filtering
            ...
            ligation_error_command = bloom_base_command + ['pairs', '--ligation-error-plot',
            ...
            re_dist_command = bloom_base_command + ['pairs', '--re-dist-plot',
    ...
    if len(pairs_files) > 0 and process_hic:
        ...
        for ix in pairs_files:
            ...
            hic_command = bloom_base_command + ['hic', '-f']
            if tmp:
    ...
    if len(hic_files) > 1:
        ...
        merge_hic_command = bloom_base_command + ['hic', '-f']
        if tmp:
    ...
    if len(hic_files) > 0:
        for ix in hic_files:
            ...
            for bin_size in bin_sizes:
                ...
                hic_command = bloom_base_command + ['hic', '-f', '-b', str(bin_size),
