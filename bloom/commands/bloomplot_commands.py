...
import bloom.plotting as kplt
import bloom.commands.bloomplot_command_parsers as parsers
...
def triangular(parameters):
    ...
    matrix = bloom.load(os.path.expanduser(args.hic), mode='r')
...
def split(parameters):
    ...
    matrix_bottom = bloom.load(os.path.expanduser(args.hic_bottom), mode='r')
    matrix_top = bloom.load(os.path.expanduser(args.hic_top), mode='r')
...
def mirror(parameters):
    ...
    matrix_upper = bloom.load(os.path.expanduser(args.hic_upper), mode='r')
    matrix_lower = bloom.load(os.path.expanduser(args.hic_lower), mode='r')
...
def scores(parameters):
    ...
    array = bloom.load(os.path.expanduser(args.scores), mode='r')
...
def line(parameters):
    ...
    regions = [bloom.load(file_name) for file_name in args.regions]
