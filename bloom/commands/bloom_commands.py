...
def bloom_parser():
    ...
    for name, function in globals().items():
        if name.endswith("_parser") and name != 'bloom_parser':
            parser = function()
    ...
    parser = argparse.ArgumentParser(
        description="bloom processing tool for Hi-C data",
    ...
    parser.add_argument(
        ...
        help='Email address for bloom command summary.'
    )
...
def auto_parser():
    import bloom.commands.auto
    return bloom.commands.auto.auto_parser()


def auto(argv, **kwargs):
    import bloom.commands.auto
    return bloom.commands.auto.auto(argv, **kwargs)
...
def map_parser():
    ...
    parser.add_argument(
        '--bloom-parallel', dest='mapper_parallel',
        action='store_false',
        default=True,
        help='Use bloom parallelisation, which launches multiple mapper jobs. '
...
def map(argv, **kwargs):
    ...
    import bloom.map as map
    from bloom.tools.general import mkdir
    from genomic_regions.files import create_temporary_copy
    from bloom.tools.files import random_name
    ...
    try:
        ...
        for input_file in input_files:
            ...
            if not args.split_fastq:
                ...
                from bloom.tools.files import split_fastq, merge_sam, gzip_splitext
                ...
                try:
                    ...
                    for split_file in split_fastq(input_file, split_fastq_tmpdir):
                        ...
                        split_command = ['bloom', 'map', split_file, index_path, split_bam_file,
...
def fragments(argv, **kwargs):
    ...
    from bloom.regions import genome_regions
    regions = genome_regions(genome_file, re_or_bin_size)

    from bloom.tools.files import write_bed
    import warnings
...
def sort_sam(argv, **kwargs):
    ...
    from genomic_regions.files import create_temporary_copy
    from bloom.tools.files import sort_natural_sam
    import tempfile
    import shutil
    ...
    try:
        if tmp:
            ...
            if output_file is not None:
                ...
                with tempfile.NamedTemporaryFile(delete=False, prefix='bloom_', suffix=extension) as f:
                    output_file = f.name
            tmp = True
...
def pairs_parser():
    ...
    parser.add_argument(
        ...
        help='IMPORTANT: The last positional argument will be '
             ...
             'and an output file, ' 
             'or an existing bloom Pairs object. ' 
...
def pairs(argv, **kwargs):
    ...
    try:
        ...
        if len(input_files) == 3:
            ...
            from bloom.tools.general import get_sam_mapper
            ...
            from bloom.pairs import BwaMemQualityFilter, BwaMemUniquenessFilter, \
                UniquenessFilter, ContaminantFilter, QualityFilter, UnmappedFilter
            from bloom.general import Mask
            ...
            from bloom.pairs import generate_pairs_split as generate_pairs
            ...
            from bloom.pairs import HicProPairGenerator, FourDNucleomePairGenerator, ReadPairs
            from bloom.regions import genome_regions
        ...
        if reset_filters:
            ...
            pairs = bloom.load(pairs_file, mode='a')
            pairs.reset_filters()
            pairs.close()
        ...
        if (statistics_file is not None or statistics_plot_file is not None or
            ...
            pairs = bloom.load(pairs_file, mode='r')
            if statistics_file is not None or statistics_plot_file is not None:
                ...
                if statistics_plot_file is not None:
                    ...
                    from bloom.plotting.statistics import summary_statistics_plot
            ...
            if re_dist_plot_file is not None:
                from bloom.plotting.statistics import restriction_site_distance_plot
                fig, ax = plt.subplots()
            ...
            if ligation_error_plot_file is not None:
                from bloom.plotting.statistics import ligation_bias_plot
                fig, ax = plt.subplots()
...
def hic_parser():
    ...
    parser.add_argument(
        ...
        help='IMPORTANT: The last positional argument will be '
             ...
             'Input files. If these are bloom Pairs objects '
             '(see "bloom pairs"), they will be turned into '
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Deep copy Hi-C file. Copies a Hi-C file to bloom format '
             'by duplicating individual bins, pixels, and bias information. '
             'Can be used to upgrade an existing bloom file with an older '
             'version or to convert Cooler or Juicer files to bloom format. '
    )
...
def hic(argv, **kwargs):
    ...
    import os
    from bloom.tools.general import str_to_int
    ...
    try:
        for input_file in input_files:
            ...
            o = None
            try:
                o = bloom.load(input_file)
                if isinstance(o, bloom.Hic) or isinstance(o, bloom.hic.LegacyHic):
                    hic_files.append(input_file)
                elif isinstance(o, bloom.ReadPairs):
                    pairs_files.append(input_file)
                else:
                    from bloom.compatibility.cooler import CoolerHic
                    from bloom.compatibility.juicer import JuicerHic
        ...
        if len(pairs_files) > 0:
            ...
            for pairs_file in pairs_files:
                with bloom.load(pairs_file) as pairs:
        ...
        if len(hic_files) > 1:
            ...
            try:
                for hic_file in hic_files:
                    hics.append(bloom.load(hic_file))

                tmp_output_file = tempfile.NamedTemporaryFile(suffix='.hic', delete=False)
                merged_hic_file = tmp_output_file.name
                tmp_input_files.append(merged_hic_file)
                merged_hic = bloom.Hic.merge(hics, file_name=merged_hic_file, mode='w')
                merged_hic.close()
            finally:
                for hic in hics:
                    hic.close()
            ...
            if deepcopy:
                ...
                merged_hic_file = tmp_output_file.name
                tmp_input_files.append(merged_hic_file)
                hic = bloom.load(hic_files[0])
                copy_hic = hic.deepcopy(target_class=bloom.Hic, file_name=merged_hic_file, mode='w')
                copy_hic.close()
            else:
                merged_hic_file = hic_files[0]

        if bin_size is not None:
            merged_hic = bloom.load(merged_hic_file)
            ...
            binned_hic = bloom.load(merged_hic_file, mode='a')
        ...
        filters = []
        if filter_low_coverage_auto:
            from bloom.hic import LowCoverageFilter
        ...
        if filter_diagonal is not None:
            from bloom.hic import DiagonalFilter
        ...
        if statistics_file is not None or statistics_plot_file is not None or marginals_plot_file is not None:
            ...
            if marginals_plot_file is not None:
                from bloom.plotting.statistics import marginals_plot
...
def from_juicer_parser():
    parser = argparse.ArgumentParser(
        prog="bloom from_juicer",
        description='[deprecated] Convert Juicer Hi-C file to bloom format'
    )
    ...
    parser.add_argument(
        ...
        help='Path to juicer jar. You can also specify this in bloom.conf'
    )
...
def from_juicer(argv, **kwargs):
    ...
    try:
        ...
        from bloom.compatibility.juicer import convert_juicer_to_hic
...
def from_txt(argv, **kwargs):
    ...
    try:
        ...
        from bloom.compatibility.txt import load_regions, load_contacts
        import bloom
        regions, ix_converter = load_regions(regions_file)

        hic = bloom.Hic(file_name=output_file, mode='w')
...
def to_bloom_parser():
    parser = argparse.ArgumentParser(
        prog="bloom from-cooler",
        description="Convert a matrix file (Cooler, Juicer) file to bloom format."
    )

    parser.add_argument(
        'input',
        help="Input .cool, .mcool, or Juicer .hic file. You must specify the "
             "resolution you want to export to bloom using the format "
...
def to_bloom(argv, **kwargs):
    parser = to_bloom_parser()
    ...
    try:
        ...
        with bloom.load(input_file, mode='r', tmpdir=tmp) as mat:
            mat.deepcopy(bloom.Hic, file_name=output_file, mode='w', inter_chromosomal=inter_chromosomal)
    finally:
        if tmp:
            shutil.copy(output_file, original_output_file)
...
def from_cooler_parser():
    parser = argparse.ArgumentParser(
        prog="bloom from-cooler",
        description="[deprecated] Convert a Cooler file to bloom format."
    )
...
def from_cooler(argv, **kwargs):
    ...
    try:
        ...
        with bloom.load(input_file, mode='r', tmpdir=tmp) as cool:
            cool.deepcopy(bloom.Hic, file_name=output_file, mode='w')
    finally:
        if tmp:
            shutil.copy(output_file, original_output_file)
        for tmp_file in tmp_files:
...
def to_cooler_parser():
    ...
    parser.add_argument(
        'input',
        help='''Input binned .hic file, bloom format.'''
    )
    ...
    parser.add_argument(
        ...
        help='Do not sort regions by their natural chromosome order. '
             ...
             'file in the order they are listed in the bloom file.'
    )
...
def to_cooler(argv, **kwargs):
    ...
    try:
        ...
        parser.error("Cannot import cooler. Install cooler with 'pip install cooler'.")
    from bloom.compatibility.cooler import to_cooler
    ...
    try:
        ...
        with bloom.load(input_file, mode='r', tmpdir=tmp) as hic:
...
def to_juicer_parser():
    ...
    parser.add_argument(
        'input',
        nargs='+',
        help='Input .pairs file(s), bloom format.'
    )
...
def to_juicer(argv, **kwargs):
    ...
    import bloom
    from bloom.tools.general import str_to_int
    from bloom.compatibility.juicer import to_juicer
    if resolutions is not None:
        resolutions = [str_to_int(r) for r in resolutions]

    pairs = [bloom.load(f, tmpdir=tmp) for f in input_files]
    try:
...
def dump(argv, **kwargs):
    ...
    if subset_string is not None:
        ...
        row_subset_region = bloom.GenomicRegion.from_string(row_subset_string)
        if col_subset_string is not None:
            col_subset_region = bloom.GenomicRegion.from_string(col_subset_string)
    ...
    try:
        ...
        with bloom.load(hic_file, mode='r', tmpdir=tmp) as hic:
...
def pca(argv, **kwargs):
    ...
    try:
        if len(input_files) > 1:
            matrices = [bloom.load(file_name, tmpdir=tmp) for file_name in input_files]

            from bloom.architecture.comparisons import hic_pca
        ...
        if plot_file is not None:
            ...
            import matplotlib
            matplotlib.use("agg")
            from bloom.plotting.statistics import pca_plot
...
def loops_parser():
    parser = argparse.ArgumentParser(
        prog="bloom loops",
        description='Call loops in a Hic object using bloom '
    ...
    parser.add_argument(
        ...
        help='Output file. If input file is already '
             'a bloom compatible loops object, filtering '
    ...
    parser.add_argument(
        ...
        help='Run on SGE cluster. This option is highly '
             'recommended if you are running "bloom loops" on '
...
def loops(argv, **kwargs):
    ...
    matrix = None
    try:
        matrix = bloom.load(input_file, mode='r', tmpdir=tmp)
        is_rh_peaks = isinstance(matrix, bloom.peaks.RaoPeakInfo)
        is_merged_peaks = isinstance(matrix, bloom.peaks.PeakInfo)
        is_matrix = isinstance(matrix, bloom.matrix.RegionMatrixContainer)
        ...
        if is_matrix and not is_rh_peaks and not is_merged_peaks:
            pk = bloom.peaks.RaoPeakCaller(p=peak_size, w_init=width, min_locus_dist=peak_size,
        ...
        if is_rh_peaks and not is_merged_peaks:
            if not ran_peak_calling and not merge and output_file is not None:
                ...
                matrix = bloom.load(output_file, mode='a')
            ...
            if (fdr_cutoff_global is not None or fdr_cutoff_donut is not None or
                ...
                fdr_filter = bloom.peaks.FdrPeakFilter(fdr_cutoff=fdr_cutoff_global,
            ...
            if min_dist is not None:
                if peak_size is None or min_dist > peak_size:
                    ...
                    distance_filter = bloom.peaks.DistancePeakFilter(min_dist,
                                                                    mask=distance_mask)
            ...
            if min_observed is not None:
                ...
                observed_filter = bloom.peaks.ObservedPeakFilter(min_observed,
                                                                mask=observed_mask)
                filters.append(observed_filter)

            if rh_filter:
                rh_mask = matrix.add_mask_description('rao_huntley', "Rao Huntley filter")
                rh = bloom.peaks.RaoPeakFilter(mask=rh_mask)
                filters.append(rh)
        ...
        if is_merged_peaks:
            ...
            if remove_singlets:
                ...
                rao_filter = bloom.peaks.RaoMergedPeakFilter(mask=mask)
            ...
            if fdr_sum is not None:
                ...
                fdr_sum_filter = bloom.peaks.FdrSumFilter(cutoff=fdr_sum, mask=mask)
...
def overlap_peaks(argv, **kwargs):
    ...
    peaks = [bloom.load(i, mode="r") for i in input_paths]
...
def boundaries(argv, **kwargs):
    ...
    import bloom
    from bloom.tools.general import human_format, str_to_int
    import genomic_regions as gr
    from bloom.architecture.domains import InsulationScores, InsulationScore, Boundaries
    from bloom.architecture.comparisons import ComparisonScores

    insulation = None
    try:
        insulation = bloom.load(input_file, mode='r')
...
def compare(argv, **kwargs):
    ...
    import bloom
    from bloom.architecture.comparisons import FoldChangeMatrix, DifferenceMatrix, NonzeroFilter, \
        FoldChangeScores, DifferenceScores, FoldChangeRegions, DifferenceRegions
    from bloom.matrix import RegionMatrixContainer
    from bloom.architecture.domains import RegionScoreParameterTable
    from genomic_regions import RegionBased

    matrix1 = bloom.load(input_file1, mode='r')
    matrix2 = bloom.load(input_file2, mode='r')
    ...
    if isinstance(matrix1, RegionMatrixContainer) and isinstance(matrix2, RegionMatrixContainer):
        ...
        if output_format == 'bed':
            ...
            from bloom.tools.files import write_bigwig
            write_bigwig(output_file, cmp.regions)
        elif output_format == 'gff':
            from bloom.tools.files import write_gff
...
def directionality_parser():
    ...
    parser.add_argument(
        ...
        help='Output file. Format will be determined by '
             '"-o". By default, this is a bloom DirectionalityIndexes '
    ...
    parser.add_argument(
        ...
        help='Format of the output file. '
             'By default, this is a bloom DirectionalityIndex '
...
def directionality(argv, **kwargs):
    ...
    from bloom.architecture.domains import DirectionalityIndexes
    import os
...
def insulation_parser():
    ...
    parser.add_argument(
        ...
        nargs='?',
        help='Output file. Format will be determined by '
             '"-o". By default, this is a bloom InsulationScores '
             'object, for maximum compatibility with other analyses. '
             'If you choose a text-based output format (BED, GFF, '
             'BigWig), this parameter will be the file prefix, and '
             'the window size will be appended.'
    ...
    parser.add_argument(
        ...
        help='Format of the output file. '
             'By default, this is a bloom InsulationScore '
             'object, for maximum compatibility with other '
             'analyses. Other options are "bed", "bigwig", '
             'and "gff"'
    )
    ...
    parser.add_argument(
        ...
        help='Use geometric mean for normalisation (rather than arithmetic mean). '
             ...
             'using the "bloom compare" command!'
    )
...
def insulation(argv, **kwargs):
    ...
    from bloom.architecture.domains import InsulationScores
    import os
...
def _domain_scores(parser, input_file, output_file, output_format, default_output_format,
                   ...
                   **kwargs):
    import bloom
    from bloom.tools.general import str_to_int, human_format
    output_format = output_format.lower()
    ...
    scores = None
    try:
        matrix = bloom.load(input_file, mode='r')
...
def compartments(argv, **kwargs):
    ...
    from bloom.tools.files import write_bed
    from bloom.matrix import RegionMatrixContainer
    from bloom.architecture.compartments import ABCompartmentMatrix

    matrix = None
    ab_matrix = None
    try:
        matrix = bloom.load(input_file, tmpdir=tmp)
        # input is Hic or other matrix
        if not isinstance(matrix, ABCompartmentMatrix):
            if not isinstance(matrix, RegionMatrixContainer):
                parser.error("Input must be bloom matrix (e.g. Hic)")
            ...
            if os.path.exists(output_file) and not recalculate:
                ab_matrix = bloom.load(output_file)
            ...
            matrix.close()
            matrix = None
            ab_matrix = bloom.load(input_file, mode='a', tmpdir=tmp)

        # calculate eigenvector
        ev = None
        if eigenvector_file is not None:
            if os.path.exists(eigenvector_file) and not force:
                ev_regions = bloom.load(eigenvector_file)
        ...
        if matrix_file is not None or enrichment_file is not None:
            ...
            if enrichment_file is not None:
                ...
                from bloom.plotting.statistics import saddle_plot
...
def expected(argv, **kwargs):
    ...
    for label, input_file in zip(labels, input_files):
        with bloom.load(input_file, mode='a', tmpdir=tmp) as matrix:
    ...
    if plot_file is not None:
        ...
        matplotlib.use("agg")
        import matplotlib.pyplot as plt
        from bloom.plotting.statistics import distance_decay_plot

        hics = [bloom.load(file_name) for file_name in input_files]
...
def subset(argv, **kwargs):
    ...
    old_hic = bloom.load(input_file, mode='r')
    if isinstance(old_hic, JuicerHic) or isinstance(old_hic, CoolerHic):
        parser.error("Cannot use subset on Juicer or Cooler files. Use bloom format if you want to subset.")
...
def aggregate_parser():
    parser = argparse.ArgumentParser(
        prog="bloom aggregate",
        description='Make aggregate plots with bloom'
    )

    parser.add_argument(
        'input',
        help='bloom matrix file (e.g. Hic)'
    )
...
def aggregate(argv, **kwargs):
    ...
    import genomic_regions as gr
    import warnings
    from bloom.architecture.aggregate import AggregateMatrix, loop_strength, tad_strength, loop_strength_by_region_pair
    from bloom.tools.general import human_format, str_to_int
    ...
    regions = None
    try:
        with bloom.load(input_file, mode='r') as matrix:
            if not isinstance(matrix, AggregateMatrix):
                regions = bloom.load(regions_file)
            ...
            if plot_file is not None:
                ...
                matplotlib.use('agg')
                import matplotlib.pyplot as plt
                from bloom.plotting.statistics import aggregate_plot
...
def stats_parser():
    parser = argparse.ArgumentParser(
        prog="bloom stats",
        description='[deprecated] Get statistics on bloom pipeline steps.'
    )
...
def stats(argv, **kwargs):
    ...
    if args.hic is not None:
        ...
        for hic_file in hic_files:
            ...
            hic = bloom.load(hic_file, mode='r')
...
def cis_trans(argv, **kwargs):
    ...
    import bloom
    from bloom.architecture.stats import cis_trans_ratio
...
def downsample(argv, **kwargs):
    ...
    print("*** bloom downsample is deprecated. Please use bloom hic --downsample instead! ***")
    ...
    try:
        ...
        with bloom.load(hic_file) as hic:
...
def upgrade_parser():
    parser = argparse.ArgumentParser(
        prog="bloom upgrade",
        description='Upgrade objects from old bloom versions.'
    )
...
def upgrade(argv, **kwargs):
    ...
    import bloom
    import numpy as np
    from bloom.hic import LegacyHic
    import tables
    from bloom.tools.general import RareUpdateProgressBar
    ...
    try:
        ...
        try:
            old_bloom = bloom.load(input_file, mode='a')

            if isinstance(old_bloom, LegacyHic):
                import bloom

                if output_file is None:
                    raise ValueError("Must provide an output file to upgrade to latest Hi-C version!")

                new_hic = bloom.Hic(file_name=output_file, mode='w')
                new_hic.add_regions(old_bloom.regions)

                bv = [row['bias']**2 for row in old_bloom.file.get_node('/', 'node_annot').iterrows()]

                with RareUpdateProgressBar(prefix="Upgrade", max_value=len(old_bloom.edges)) as pb:
                    for i, edge in enumerate(old_bloom.edges(lazy=True)):
                        source = edge.source
                        sink = edge.sink
                ...
                new_hic.close()
                return
            elif isinstance(old_bloom, bloom.ReadPairs) or isinstance(old_bloom, bloom.Hic):
                if old_bloom._chromosomes_info is None:
                    if output_file is not None:
                        raise ValueError("Can only do upgrade in place for chromosome table")
                    logger.info("Updating chromosome info table")
                    old_bloom._update_chromosomes_info()
                    old_bloom.close()
                return
        except (ValueError, TypeError):
            pass

        file_based = bloom.FileBased(input_file, mode='r')
        ...
        if class_id == 'ACCESSOPTIMISEDHIC' or class_id == 'HIC':
            target_class = bloom.Hic
        elif class_id == 'RAOPEAKINFO':
            target_class = bloom.RaoPeakInfo
        else:
