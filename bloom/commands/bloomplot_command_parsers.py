...
def bloomplot_parser():
    usage = '''bloomplot [<bloomplot global parameters>] <region> [<region> ...]
            --plot <plot type> [<plot parameters>] <plot data file(s)> [...]

            Run bloomplot --plot <plot type> -h for help on a specific subplot.\n\nPlot types:\n\n'''

    command_descriptions = dict()
    for name, function in globals().items():
        if name.endswith("_parser") and name != 'bloomplot_parser':
            parser = function()
    ...
    parser = argparse.ArgumentParser(
        description="bloomplot plotting tool for bloom",
...
def type_parser():
    parser = argparse.ArgumentParser(
        description="bloomplot subplot identifier",
        add_help=False,
    ...
    parser.add_argument(
        'type',
        help='Plot type. See bloomplot -h for options.'
    )
...
def subplot_parser(plot_type='<plot type>'):
    parser = argparse.ArgumentParser(
        prog='bloomplot <region> -p {}'.format(plot_type),
        description="bloomplot subplot identifier",
    )
