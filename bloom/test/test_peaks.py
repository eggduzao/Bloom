...
import bloom
from bloom.peaks import RaoPeakCaller, RaoPeakInfo
from bloom.hic import Hic
from bloom.matrix import RegionMatrix
from genomic_regions import GenomicRegion
from bloom.tools.general import pairwise
import numpy as np
import math
import pickle
import os.path
...
class TestRaoPeakCaller:
    ...
    def test_call_peaks(self):
        ...
        hic_10kb = bloom.load(dir + "/test_peaks/rao2014.chr11_77400000_78600000.hic", mode='r')
...
class TestOverlapPeaks:
    def setup_method(self, method):
        ...
        self.peaks = {}
        for i in range(3):
            p = bloom.peaks.PeakInfo()
            p.add_regions(regions)
            edges = [bloom.peaks.Peak(x=x, y=y, source=math.floor(x), sink=math.floor(y), weight=1)
    ...
    def test_overlap(self):
        stats, merged = bloom.peaks.overlap_peaks(self.peaks, max_distance=100)
