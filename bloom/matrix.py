...
class Edge(object):
    ...
    .. attribute:: source_node
        ...
        The first :class:`~bloom.GenomicRegion` in this contact

    .. attribute:: sink_node

        The second :class:`~bloom.GenomicRegion` in this contact
    """
    def __init__(self, source, sink, _weight_field='weight', **kwargs):
        """
        :param source: The index of the "source" genomic region
                       or :class:`~bloom.GenomicRegion` object.
...
def as_edge(edge):
    ...
    :param edge: Can be :class:`~Edge`,
                 ...
                 tuple of the form (:class:`~bloom.GenomicRegion`, :class:`~bloom.GenomicRegion`),
...
class RegionPairsContainer(RegionBased):
    ...
    therefore based on a list of genomic regions (:class:`~bloom.GenomicRegion`)
    ...
    * :func:`~RegionPairsContainer._edges_subset` is also used by
      ...
      two lists of :class:`~bloom.GenomicRegion` objects, :code:`row_regions` and
    ...
    * :func:`~RegionPairsContainer._edges_length` which returns the total number of
    ...
    def edges(self):
        ...
        If the key is a string or a :class:`~bloom.GenomicRegion`, all non-zero edges connecting
        the region described by the key to any other region are returned. If the key is a
        tuple of strings or :class:`~bloom.GenomicRegion`, only edges between the two regions
        are returned.
        ...
        which can be slow when iterating over a lot of edges. This is why all file-based bloom
        ...
        .. code ::
            ...
            for edge in hic.edges('chr18', lazy=True):
                ...
                # 42 42 0.12291311562018173 <bloom.matrix.LazyEdge for row /edges/chrpair_0_0.row (Row), pointing to row #0>
                # 24 28 0.025205961072838057 <bloom.matrix.LazyEdge for row /edges/chrpair_0_0.row (Row), pointing to row #1>
                # 5 76 0.00961709840049876 <bloom.matrix.LazyEdge for row /edges/chrpair_0_0.row (Row), pointing to row #2>
                # 66 68 0.03876763316345468 <bloom.matrix.LazyEdge for row /edges/chrpair_0_0.row (Row), pointing to row #3>
                # ...
                ...
                if oe:
                    ...
                    if not oe_per_chromosome and expected_genome is None:
                        raise ValueError("Expected values were not calculated for the whole genome. "
                                         ...
                                         "is only supported for bloom Hi-C files. If this is a bloom file, "
...
class RegionMatrixContainer(RegionPairsContainer, RegionBasedWithBins):
    ...
    :class:`~bloom.Hic` or :class:`~bloom.FoldChangeMatrix`. It provides
    ...
    .. code ::
        ...
        m = hic.matrix()
        type(m)  # bloom.matrix.RegionMatrix
    ...
    def matrix(self, key=None,
        ...
        :param key: Matrix selector. See :func:`~bloom.matrix.RegionPairsContainer.edges`
        ...
        :param args: Positional arguments passed to
                     :func:`~bloom.matrix.RegionMatrixContainer.regions_and_matrix_entries`
        :param kwargs: Keyword arguments passed to
                       :func:`~bloom.matrix.RegionMatrixContainer.regions_and_matrix_entries`
        :return: :class:`~bloom.matrix.RegionMatrix`
        """
...
class RegionPairsTable(RegionPairsContainer, Maskable, RegionsTable):
    ...
    def filter(self, edge_filter, queue=False, log_progress=not config.hide_progressbars):
        ...
        :class:`~bloom.general.MaskFilter`.

        :param edge_filter: Class implementing :class:`~bloom.general.MaskFilter`.
        ...
        if hasattr(self, '_expected_value_group') and self._expected_value_group is not None:
            try:
                ...
                warnings.warn("Matrix file opened in read-only mode, "
                              ...
                              "Run 'bloom expected <matrix_file>' on the "
...
class RegionMatrix(np.ma.MaskedArray):
    ...
    :class:`~RegionMatrix` supports subsetting by :class:`~bloom.GenomicRegion`
    ...
    .. code::
        ...
        type(m_sub)  # bloom.matrix.RegionMatrix
        m.shape  # 5, 10
