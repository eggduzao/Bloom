...
bloom pipeline, a Hic object is assembled at the fragment level from
:class:`~bloom.construct.seq.FragmentMappedReadPairs`. From there, it can be
...
import pysam
import pybedtools
from bloom.tools.files import is_fasta_file, write_bigwig, write_bed, write_gff
from bloom.tools.matrix import apply_sliding_func
from bloom.tools.general import natural_sort, natural_cmp
from Bio import SeqIO, Restriction, Seq
from bloom.legacy.data.general import TableObject, Maskable, MaskedTable, MaskFilter, FileGroup
from abc import abstractmethod, ABCMeta
import os.path
from bloom.tools.general import ranges, distribute_integer, create_col_index, \
...
class RegionsTable(GenomicRegions, FileGroup):
    ...
    (:class:`~bloom.construct.seq.FragmentMappedReadPairs`)
    """
    ...
    def _subset_rows(self, key):
        ...
        :param key: A :class:`~bloom.data.genomic.GenomicRegion` object,
...
class RegionMatrixTable(RegionPairs):
    ...
    def _impute_missing_contacts(self, hic_matrix=None, _expected_contacts=None):
        ...
        from bloom.architecture.hic_architecture import ExpectedContacts
...
class Hic(RegionMatrixTable):
    ...
    def load_read_fragment_pairs(self, pairs):
        """
        Load data from :class:`~bloom.construct.seq.FragmentMappedReadPairs`.

        This method automatically sums up reads mapping to the same
        fragment pairs and creates exactly one edge per fragment pair.

        :param pairs: A :class:`~bloom.construct.seq.FragmentMappedReadPairs`
                      object.
        """
        # add regions
        if len(self._regions) != 0:
    ...
    def architecture(self):
        import bloom.legacy.architecture.hic_architecture as ha
