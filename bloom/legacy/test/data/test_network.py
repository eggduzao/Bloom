...
import bloom
from bloom.data.network import RaoPeakCaller, RaoPeakInfo
from bloom.data.genomic import Hic, RegionMatrix, GenomicRegion
...
class TestOverlapPeaks:
    def setup_method(self, method):
        ...
        self.peaks = {}
        for i in range(3):
            p = bloom.data.network.PeakInfo()
            p.add_regions(regions)
            p.add_edges(bloom.data.network.Peak(x=x, y=y, source=math.floor(x), sink=math.floor(y)) for x, y in (tuple(sorted(xy)) for xy in peaks[i]))
            self.peaks[i] = p

    def test_overlap(self):
        stats, merged = bloom.data.network.overlap_peaks(self.peaks, max_distance=100)
        expected = {
            (0, 1, 2): 1,
            (0,): 1,
            (1, 2): 1,
