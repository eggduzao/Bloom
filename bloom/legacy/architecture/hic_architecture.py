...
from bloom.legacy.architecture.architecture import TableArchitecturalFeature, calculateondemand, \
    ArchitecturalFeature
from bloom.legacy.architecture.genome_architecture import MatrixArchitecturalRegionFeature, \
    VectorArchitecturalRegionFeature,  MatrixArchitecturalRegionFeatureFilter
from bloom.architecture.maxima_callers import MaximaCallerDelta
from genomic_regions import Bedpe
from bloom.regions import GenomicRegion, Genome
from bloom.matrix import Edge
from bloom.hic import Hic, HicEdgeFilter
from collections import defaultdict
from bloom.tools.general import ranges, to_slice
from bloom.tools.matrix import apply_sliding_func, kth_diag_indices, trim_stats
from bloom.general import FileGroup
...
def cis_trans_ratio(hic, normalise=False):
    ...
    :param hic: :class:`~bloom,data.genomic.Hic` object
...
class HicEdgeCollection(MatrixArchitecturalRegionFeature):
    ...
    def __init__(self, hics=None, additional_fields=None, file_name=None, mode='a', tmpdir=None,
        ...
        :param hics: Iterable of :class:`~bloom,data.genomic.Hic` objects
...
class ExpectedContacts(TableArchitecturalFeature):
    ...
    :param hic: A :class:`~bloom.data.genomic.RegionMatrixTable` object
                    such as :class:`~bloom.data.genomic.Hic`
    ...
    :param min_reads: Minimum number of reads in sliding window to apply smooting
                      function
    :param regions: A region selector string, :class:`~bloom.data.genomic.GenomicRegion`, or lists thereof.
    :param weight_column: Name of the column containing the weights/values for the
                          ...
                          in the provided :class:`~bloom.data.genomic.RegionMatrixTable`
...
class ObservedExpectedRatio(MatrixArchitecturalRegionFeature):
    """
    Calculate the ratio of observed over expected contacts in a :class:`~bloom.data.genomic.RegionMatrixTable`.
    ...
    :param weight_column: Name of the column containing the weights/values for the
                          expected value calculation. If None, this will be the default field
                          in the provided :class:`~bloom.data.genomic.RegionMatrixTable`
    :return: :class:`~bloom.architecture.genome_architecture.MatrixArchitecturalRegionFeature`
    """
...
class ComparisonMatrix(MatrixArchitecturalRegionFeature):
    """
    Compare two :class:`~bloom.data.genomic.RegionMatrixTable` objects.

    Define edge comparison function manually.

    :param matrix1: :class:`~bloom.data.genomic.RegionMatrixTable`, such as :class:`~bloom.data.genomic.Hic`
    :param matrix2: :class:`~bloom.data.genomic.RegionMatrixTable`, such as :class:`~bloom.data.genomic.Hic`
    :param file_name: Path to save file location
    :param mode: File mode ('r' = read-only, 'w' = (over)write, 'a' = append)
    :param tmpdir: Temporary directory
    :param regions: A region selector string, :class:`~bloom.data.genomic.GenomicRegion`, or lists thereof.
...
class FoldChangeMatrix(ComparisonMatrix):
    """
    Calculate the fold-change matrix of two :class:`~bloom.data.genomic.RegionMatrixTable` objects.
...
class ABDomainMatrix(MatrixArchitecturalRegionFeature):
    ...
    :param hic: A :class:`~bloom.data.genomic.Hic` matrix
    :param file_name: Path to save file location
    :param mode: File mode ('r' = read-only, 'w' = (over)write, 'a' = append)
    :param tmpdir: Temporary directory
    :param regions: A region selector string, :class:`~bloom.data.genomic.GenomicRegion`, or lists thereof.
...
class ABDomains(VectorArchitecturalRegionFeature):
    ...
    :param data: A :class:`~bloom.data.genomic.Hic` object or :class:`~ABDomainMatrix`
    :param file_name: Path to save file location
    :param mode: File mode ('r' = read-only, 'w' = (over)write, 'a' = append)
    :param tmpdir: Temporary directory
...
class PossibleContacts(TableArchitecturalFeature):
    ...
    :class:`~bloom.data.genomic.RegionMatrixTable`. This is a combinatorial approach
    that also takes into account unmappable/masked regions in the genome.

    :param hic: :class:`~bloom.data.genomic.RegionMatrixTable`
    :param file_name: Path to save file location
    :param mode: File mode ('r' = read-only, 'w' = (over)write, 'a' = append)
    :param tmpdir: Temporary directory
    :param regions: A region selector string, :class:`~bloom.data.genomic.GenomicRegion`, or lists thereof.
...
class RowRegionMatrix(np.ndarray):
    ...
    def region_bins(self, region):
        ...
        :param region: A :class:`~bloom.data.genomic.GenomicRegion` object or region selector string
        :return: slice
        """
...
class MultiVectorArchitecturalRegionFeature(VectorArchitecturalRegionFeature):
    ...
    def as_matrix(self, regions=None, keys=None):
        ...
        :param regions: If None, will default to all regions in the matrix. Else provide an iterator
                        over :class:`~bloom.data.genomic.GenomicRegion`s
...
class DirectionalityIndex(MultiVectorArchitecturalRegionFeature):
    ...
    :param hic: :class:`~bloom.data.genomic.RegionMatrixTable`, typically
                a :class:`~bloom.data.genomic.Hic`object
    :param file_name: Path to save file location
    :param mode: File mode ('r' = read-only, 'w' = (over)write, 'a' = append)
    :param tmpdir: Path to temporary directory
    :param weight_column: Name of the column containing the weights/values for the
...
class InsulationIndex(MultiVectorArchitecturalRegionFeature):
    ...
    def boundaries(self, window_size, min_score=None, delta_window=3, log=False, sub_bin_precision=False,
        ...
        :return: list of :class:`~bloom.data.genomic.GenomicRegion`
        """
...
class RegionContactAverage(MultiVectorArchitecturalRegionFeature):
    ...
    :param matrix: :class:`~bloom.data.genomic.RegionMatrixTable`, typically
                   a :class:`~bloom.data.genomic.Hic`object
    :param file_name: Path to save file location
    :param mode: File mode ('r' = read-only, 'w' = (over)write, 'a' = append)
    :param tmpdir: Path to temporary directory
