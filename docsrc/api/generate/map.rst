...
To map FASTQ files using the API, we will use the :mod:`bloom.map` module.
...
read should be truncated and re-mapped to the genome. The :class:`~bloom.map.SimpleBowtie2Mapper`,
for example, only attempts to align a read once and does no iterative mapping. The
:class:`~bloom.map.Bowtie2Mapper` resubmits unaligned reads and reads with a low score,
There are equivalent mappers for BWA named
:class:`~bloom.map.SimpleBWAMapper` and :class:`~bloom.map.BWAMapper`, but here we
choose the :class:`~bloom.map.Bowtie2Mapper`. It requires only the path of the corresponding
``bowtie2`` index:
...
Now we can use :func:`bloom.map.iterative_mapping` to start the actual mapping process:
