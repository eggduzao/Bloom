...
into a :class:`~bloom.pairs.ReadPairs` object.

*******************
Obtaining fragments
*******************

:class:`~bloom.pairs.ReadPairs` map reads to restriction fragments, so first of all we are
going to need a list of those. The :func:`bloom.regions.genome_regions` function has been
...
(:class:`~bloom.pairs.QualityFilter`) or with multiple mapping locations throughout
the genome (:class:`~bloom.pairs.UniquenessFilter`). These filters (of class
:class:`~bloom.pairs.ReadFilter`) have to be added already at the read pair import step,
...
.. warning::
    ...
    these filters specific for BWA-mapped reads (:class:`~bloom.pairs.BwaMemQualityFilter`
    and :class:`~bloom.pairs.BwaMemUniquenessFilter`). Make sure you use the appropriate
    filter for your chosen mapper, as the quality and uniqueness definitions used by each
    application differ substantially!

The :class:`~bloom.pairs.QualityFilter` removes all read pairs where one or both reads have
a MAPQ value lower than ``30`` (in this case). The :class:`~bloom.pairs.UniquenessFilter`
...
For importing read pairs from SAM files we can use :func:`~bloom.pairs.generate_pairs_split`.
...
The resulting object is of the type :class:`~bloom.pairs.ReadPairs`, which implements
...
As you can see, :class:`~bloom.pairs.ReadPairs` is a container for
:class:`~bloom.pairs.FragmentReadPair` objects. Each object contains information about its
two associated reads in the :py:attr:`~bloom.pairs.FragmentReadPair.left` and
:py:attr:`~bloom.pairs.FragmentReadPair.right` properties, respectively, which return a
:class:`~bloom.pairs.FragmentRead` object.

:class:`~bloom.pairs.FragmentRead` objects contain information about the read's mapping
...
subsets with the :func:`bloom.pairs.ReadPairs.pairs` function:
...
The :func:`bloom.pairs.ReadPairs.pairs` function supports :ref:`lazy evaluation <lazy_evaluation>`
...
bloom offers a number of :class:`~bloom.pairs.FragmentReadPairFilter` to remove problematic pairs.
...
a fresh start, which is called :func:`~bloom.pairs.ReadPairs.reset_filters`:
...
We will illustrate how filters are used with the :class:`~bloom.pairs.SelfLigationFilter`.
...
filter name. We can also pass it a more elaborate :class:`~bloom.general.Mask` object with
...
:func:`~bloom.pairs.ReadPairs.filter` with the corresponding filter instance.
...
Once the :func:`~bloom.pairs.ReadPairs.filter` command has completed, the filtered pairs will
...
To obtain a dictionary with the filter statistics, use :func:`~bloom.pairs.ReadPairs.filter_statistics`
...
Here are the filters available in bloom for read/fragment pairs:

- :class:`~bloom.pairs.SelfLigationFilter` removes all pairs where both reads map to the same fragment
- :class:`~bloom.pairs.ReDistanceFilter` can filter for an expected DNA (not restriction) fragment
...
- :class:`~bloom.pairs.PCRDuplicateFilter` will find pairs mapping to the exact same genomic locations
  (both reads), and will only retain one copy of the exact duplicates.
- :class:`~bloom.pairs.InwardPairsFilter` and :class:`~bloom.pairs.OutwardPairsFilter` are removing
...
Next, we will convert the filtered pairs to a :class:`~bloom.hic.Hic` object.
