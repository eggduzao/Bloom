...
hic_100kb = bloom.load("output/hic/binned/bloom_example_100kb.hic")
# end snippet domains setup
import matplotlib.pyplot as plt

# start snippet alternative cooler
hic_100kb = bloom.load("architecture/other-hic/bloom_example.mcool@100kb")
# end snippet alternative cooler

# start snippet alternative juicer
hic_100kb = bloom.load("architecture/other-hic/bloom_example.juicer.hic@100kb")
# end snippet alternative juicer

# start snippet insulation basic
insulation = bloom.InsulationScores.from_hic(hic_100kb,
                                            [5000000, 1000000, 1500000,
                                             2000000, 2500000, 3000000,
                                             3500000, 4000000],
                                            file_name="architecture/domains/bloom_example_100kb.insulation")
...
insulation.to_bed("architecture/domains/bloom_example_100kb.insulation_1mb.bed", 1000000)
# end snippet insulation export

fig, ax = plt.subplots(figsize=(6, 3))
# start snippet insulation multiplot
p = bloomplot.GenomicVectorArrayPlot(insulation, colormap='RdBu_r', vmin=-1, vmax=1,
                                    genomic_format=True)
...
p = bloomplot.LinePlot(insulation, ylim=(-1, 1), colors=['darkturquoise'], style="mid",
...
boundaries = bloom.Boundaries.from_insulation_score(insulation, window_size=1000000)
...
ph = bloomplot.TriangularMatrixPlot(hic_100kb, max_dist=5000000, vmin=0, vmax=0.05)
pb = bloomplot.BarPlot(boundaries)
f = bloomplot.GenomicFigure([ph, pb])
...
robust_boundaries = bloom.Boundaries.from_insulation_score(insulation, window_size=1000000,
                                                          min_score=1.0)
# end snippet boundaries minscore


# start snippet directionality basic
directionality = bloom.DirectionalityIndexes.from_hic(hic_100kb,
                                                     [1000000, 1500000,
                                                      2000000, 2500000, 3000000,
                                                      3500000, 4000000],
                                                     file_name="architecture/domains/bloom_example_100kb.di")
# end snippet directionality basic

fig, ax = plt.subplots(figsize=(6, 3))
# start snippet directionality multiplot
p = bloomplot.GenomicVectorArrayPlot(directionality, colormap='RdBu_r', vmin=-0.1, vmax=0.1,
                                    genomic_format=True)
p.plot('chr18:18mb-28mb')
...
p = bloomplot.LinePlot(directionality, ylim=(-0.1, 0.1), colors=['darkturquoise'], style="mid",
