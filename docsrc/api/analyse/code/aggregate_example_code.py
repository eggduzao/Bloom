...
hic_rao = bloom.load("architecture/loops/rao2014.chr11_77400000_78600000.hic")

boundaries = bloom.load("architecture/domains/bloom_example_100kb.insulation_boundaries_score_0.7_1mb.bed")
tads = bloom.load("architecture/domains/gm12878_tads.bed")
loops = bloom.load("architecture/loops/rao2014.chr11_77400000_78600000.loops_no_singlets.bedpe")
...
fixed_am = bloom.AggregateMatrix.from_center(hic_100kb, boundaries.regions,
                                            window=5000000)
# end snippet aggregate fixed

fig, ax = plt.subplots()
# start snippet aggregate plotfixed
ax = bloomplot.aggregate_plot(fixed_am, vmin=-1, vmax=1,
...
variable_am = bloom.AggregateMatrix.from_regions(hic_100kb, tads.regions)
# end snippet aggregate variable

fig, ax = plt.subplots()
# start snippet aggregate plotvariable
ax = bloomplot.aggregate_plot(variable_am, vmin=-0.5, vmax=0.5,
...
rescale_am = bloom.AggregateMatrix.from_regions(hic_100kb, tads.regions,
                                               rescale=True, log=False)
# end snippet aggregate rescale

fig, ax = plt.subplots()
# start snippet aggregate plotrescale
ax = bloomplot.aggregate_plot(rescale_am, vmax=0.045,
                             relative_label_locations=[0.33, 0.66],
                             colormap='germany')
...
loops_am = bloom.AggregateMatrix.from_center_pairs(hic_rao, loops.region_pairs())
# end snippet aggregate loops

fig, ax = plt.subplots()
# start snippet aggregate plotloops
ax = bloomplot.aggregate_plot(loops_am, vmin=-0.5, vmax=0.5,
