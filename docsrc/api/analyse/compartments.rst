...
The following steps assume that you ran the ``bloom auto`` command in :ref:`example-bloom-auto`.
...
:func:`~bloom.architecture.compartments.ABCompartmentMatrix.from_hic` function:
...
The ``ab`` object acts like any bloom matrix (see :ref:`matrix_interface`), which means you
...
.. literalinclude:: code/ab_example_code.py
    :language: python
    :start-after: start snippet ab bloomplot-correlation
    :end-before: end snippet ab bloomplot-correlation
...
:func:`~bloom.architecture.compartments.ABCompartmentMatrix.eigenvector` returns a numpy
array with one entry per region in the AB correlation matrix (you can retrieve a matching
list of regions with :func:`~bloom.architecture.compartments.ABCompartmentMatrix.regions`).
...
.. warning::
   ...
   One option implemented in bloom is to use GC content as a proxy for activity, as GC-rich
   regions have been shown to be associated with the active compartment. bloom implements the
...
To plot the EV, you can use :class:`~bloom.plotting.LinePlot`:
...
An enrichment profile, which is used to create a :func:`~bloom.plotting.saddle_plot` is used
...
In bloom, you can use the :func:`~bloom.architecture.compartments.ABCompartmentMatrix.enrichment_profile`
...
:func:`~bloom.architecture.compartments.ABCompartmentMatrix.enrichment_profile` also returns
...
You can use the :func:`~bloom.plotting.saddle_plot` function to plot the results:
