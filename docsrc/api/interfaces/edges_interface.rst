...
represented within bloom by the :class:`~bloom.pairs.ReadPairs` class. But also matrix-based
data, such as in :class:`~bloom.matrix.Hic` can be interpreted as scores between pairs of
binned genomic regions, hence it also supports the :class:`~bloom.matrix.RegionPairsContainer`
interface. After loading a dataset using :func:`~bloom.tools.load.load`, you can check for
support of the :class:`~bloom.matrix.RegionPairsContainer` interface with:
...
The current list of bloom classes supporting :class:`~bloom.matrix.RegionPairsContainer` is:
:class:`~bloom.pairs.ReadPairs`,
...
In :class:`~bloom.matrix.RegionPairsContainer` objects, the basic data type is
:class:`~bloom.matrix.Edge`. It "connects" two :class:`~genomic_regions.GenomicRegion`
...
:class:`~bloom.matrix.Hic` object edges and many other objects have a "weight"
...
If, for example, an :class:`~bloom.matrix.Edge` is created like this:
...
:class:`~bloom.matrix.RegionPairsContainer` compatible objects are built on lists of
...
edge lists can be queried using the :func:`~bloom.matrix.RegionPairsContainer.edges`
function.

In its simplest form, :func:`~bloom.matrix.RegionPairsContainer.edges` can simply be used as
...
The real power of :func:`~bloom.matrix.RegionPairsContainer.edges`, however, lies in its use as
a function:
...
in the matrix. The process of creating a unique :class:`~bloom.Matrix.Edge` object for every
matrix entry can thus cumulatively take a significant amount of time. For this reason, bloom
...
instance of the :class:`~bloom.matrix.LazyEdge` object, which simply points to the
...
is a list of the same :class:`~bloom.matrix.LazyEdge` object pointing to the same edge
data.
...
:class:`~bloom.matrix.LazyEdge` object. Lazy iterators can greatly speed up your analyses,
but **be very careful** working with them!

Another useful feature of lazy iterators is that they support data modification for
native bloom objects. For example, you double the edge weight of each edge in the
object like this:
...
error for files opened in read-only mode. The :func:`~bloom.matrix.LazyEdge.update` function
...
The :func:`~bloom.matrix.RegionPairsContainer.edges` iterator is the most versatile and useful
function in the :class:`~bloom.matrix.RegionPairsContainer` interface, but by far not the only one.
...
The :func:`~bloom.matrix.RegionPairsContainer.regions_and_edges` function returns three objects:
...
:func:`~bloom.regions.RegionBasedWithBins.regions` and :func:`~bloom.matrix.RegionPairsContainer.edges`.

~~~~~~~~~
edge_data
~~~~~~~~~

The function :func:`~bloom.matrix.RegionPairsContainer.edge_data` iterates over only a specific
...
:func:`~bloom.matrix.RegionPairsContainer.mappable` returns a boolean vector where each entry
...
The next interface builds on the :func:`~bloom.matrix.RegionPairsContainer` structure to populate
