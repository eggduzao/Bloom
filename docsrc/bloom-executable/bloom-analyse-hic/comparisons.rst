...
bloom provides a central utility named ``bloom compare`` to compare Hi-C matrices
...
When you provide ``bloom compare`` with two matrix files (e.g. Hic), these need to have
...
The output is a bloom matrix object (that can be used like a Hic object in most cases)
...
You can also use ``bloom compare`` to compare scores in any compatible region-based file,
such as BED, GFF, BigWig, or any region-based bloom output (e.g. insulation scores).
By default, the output will be a bloom :class:`~bloom.RegionsTable` object, but you can
...
The output of ``bloom insulation`` and ``bloom directionality`` for multiple window sizes
is a bloom multi-score file, which contains the insulation/directionality scores for
multiple windows sizes at once. You can directly compare all scores against each other
by providing two multi-score files as input to ``bloom compare`` - every window size will
