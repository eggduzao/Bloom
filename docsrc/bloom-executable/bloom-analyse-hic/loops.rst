...
We can use ``bloom loops`` to call loops in Hi-C matrices using the HICCUPS algorithm
...
When run like this, ``bloom loops`` does not actually call any loops, but merely returns
...
a computational cluster running Sun/Oracle Grid Engine, ``bloom loops`` can automatically
...
By default, ``bloom loops`` assumes a loop size of 25kb. This determines the area around
