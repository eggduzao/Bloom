...
.. note::
    ...
    commands using Juicer ``.hic`` files, replace ``output/hic/binned/bloom_example_1mb.hic``
    with ``architecture/other-hic/bloom_example.juicer.hic@1mb``. If you want to work with
    Cooler files in this tutorial, use ``architecture/other-hic/bloom_example.mcool@1mb``.
...
The ``bloom compartments`` command can produce a correlation matrix (AB compartment) object
from a bloom matrix file. Example:
...
We can quickly plot the correlation matrix using the ``bloomplot`` command:
...
.. image:: images/bloom_example_1mb.ab.png
...
If ``architecture/compartments/bloom_example_1mb.ab`` already exists, it will not be recalculated
...
We can plot the eigenvector using ``bloomplot``:
...
.. image:: images/bloom_example_1mb.ab_and_ev.png

``bloom compartments`` outputs the first eigenvector by default. In some cases it might be
...
you can supply a FASTA file with the genomic sequence to ``bloom compartments`` using the
``-g <fasta_file>`` argument. This is generally a good idea: ``bloom compartments`` then
...
.. image:: images/bloom_example_1mb.ab_profile.png
...
.. image:: images/bloom_example_1mb.ab_profile_sym.png
