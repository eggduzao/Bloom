...
##################################
Generating Hi-C matrices with bloom
##################################

This part of the documentation will focus primarily on ``bloom auto`` - the most versatile
command in the bloom toolkit. Its main goal is to convert any input to binned Hi-C matrices.
The following schematic will give you an overview of what file types ``bloom auto`` can handle
and how they are processed downstream.

.. image:: images/bloom-auto-schematic.png

``bloom auto`` will map reads in FASTQ (or gzipped FASTQ) files to a reference genome, generating
...
Internally, ``bloom auto`` constructs its Hi-C processing pipeline from more specialised ``bloom``
...
Each pipeline step in ``bloom auto`` is controlled by a specific set of parameters in ``bloom auto``.
...
Here is the help output for ``bloom auto``:

.. argparse::
   :module: bloom.commands.auto
   :func: auto_parser
   :prog: bloom auto
   :nodescription:
   :nodefault:
...
.. code:: python
    ...
    bloom auto <input 1> <input 2> <input 3> < ... > <output folder>


``bloom auto`` accepts any number of input files, which will be discussed below.
The last positional argument (without '-') **must always be the output folder** for
all intermediate and final bloom files. ``bloom auto`` will generate the following
...
``bloom auto`` will name its output files using the ``-n`` parameter as prefix. If this is not
...
by ``bloom auto`` - by default, it uses only a single processor. It is highly recommended to
...
option. It instructs ``bloom auto`` to perform as many calculations as possible in a temporary
directory.
...
`GitHub page <http://www.github.com/vaquerizaslab/bloom>`_ in the ``examples`` folder.
...
Now let us discuss the different input types ``bloom auto`` can handle.

===========
FASTQ input
===========

To process FASTQ files with ``bloom``, you must first have
...
Once you have these prerequisites, you can call ``bloom auto`` like this, assuming you are in the \
``examples`` folder:

.. code:: bash

    bloom auto SRR4271982_chr18_19_1.fastq.gzip SRR4271982_chr18_19_2.fastq.gzip \
              ./example_output/ -i bwa-index/hg19_chr18_19.fa \
              -g hg19_chr18_19_re_fragments.bed

The first two arguments are the paired-end FASTQ files. ``bloom auto`` works with FASTQ and gzipped
FASTQ files. In general, ``bloom auto`` assumes that two consecutive FASTQ file arguments are mate
...
.. note::
   ...
   interferes with the sorting and read pairing step in bloom. **Read names of the two mates
   must be identical**.

Following the FASTQ files as the last positional argument is the output folder
(``example_output``). ``-i`` or ``--genome-index`` instructs ``bloom auto`` to use the specified index
...
currently not supported (raise an `issue on GitHub <https://www.github.com/vaquerizaslab/bloom/issues>`_
...
sequence. bloom can automatically split these kinds of reads before mapping using the
``--split-ligation-junction`` option, which can improve mapping efficiency.

``bloom auto`` parallelises mapping by spawning multiple mapping
...
are using the ``-tmp`` option, which causes ``bloom auto`` to perform most pipeline steps in a
...
provide SAM/BAM files to ``bloom auto`` directly. This is described in the following section.
You can also perform the mapping separately with the ``bloom map`` command, which also gives you
...
A minimal ``bloom auto`` command using SAM/BAM files could look like this:

.. code:: bash

    bloom auto output/sam/SRR4271982_chr18_19_1.bam output/sam/SRR4271982_chr18_19_2.bam \
              ./example_output/ -g hg19_chr18_19_re_fragments.bed

Similarly to FASTQ input, ``bloom auto`` assumes that two consecutive SAM/BAM files represent
...
``bloom auto`` will perform an in silico digestion of the genome and use the resulting
...
.. note::
    ...
    chromosomes can negatively affect bloom performance, it is generally a good idea to limit
    the analysis to canonical chromosomes. A very easy way to do with with ``bloom`` is the
    ``bloom fragments`` command, which accepts a ``--chromosomes`` option to specify exactly
...
object. By default, ``bloom auto`` excludes unmappable and multimapping reads, as these are
unusable or misleading in interpreting Hi-C data. Additional filters for read pairs are
described in the :ref:`bloom-auto-pairs` section.

You can run the SAM/BAM to Pairs step of the ``bloom auto`` pipeline separately using
``bloom pairs``, which is described in mor detail in :ref:`bloom-pairs`.
...
text files that contain read pair information. bloom supports valid pairs files from
...
With ``bloom auto`` you can load them like this

.. code:: bash

    bloom auto test.validPairs ./example_output/ -g hg19_chr18_19_re_fragments.bed

``bloom auto`` will attempt to automatically determine if you supply a valid pairs file.
...
Pairs input
===========
If you already have a bloom Pairs object, for example from a previous ``bloom auto`` run or
from the ``bloom pairs`` command, you can feed them to ``bloom auto`` directly:

.. code:: bash

    bloom auto output/pairs/bloom_example.pairs ./example_output/

The Pairs objects already contain restriction fragment information, hence the ``-g`` parameter
is no longer necessary. Unless using the ``--no-filter-pairs`` option, ``bloom auto`` will first
...
filtering options are available in the description of the separate ``bloom pairs`` command:
:ref:`bloom-pairs`
...
If you already have a bloom Hic object, for example from a previous ``bloom auto`` run or
from the ``bloom hic`` command, you can feed them to ``bloom auto`` directly:

.. code:: bash

    bloom auto output/hic/bloom_example.hic ./example_output/
...
You can run the Hi-C processing step independently with the ``bloom hic`` command, as described
in detail in :ref:`bloom-hic`
...
Now that we have covered all the different input options fort ``bloom auto``, it is worth
stressing that you can combined different types of input in the same command. ``bloom auto``
...
.. code:: bash
    ...
    bloom auto hic/test.hic pairs/test.pairs test.validPairs \
...
By default, ``bloom auto`` runs tasks in parallel locally on the machine it was started on.
If you want to perform a test run, without actually executing any commands, you can use
the ``--run-with test`` option. This will not run any of the ``bloom`` pipeline steps, but
...
can instruct ``bloom auto`` to submit all commands to the cluster using ``--run-with sge``.
...
Once you have generated your binned, filtered, and corrected Hic objects with ``bloom auto``,
you may want to explore the data in those matrices. bloom provides a number of commands for
data analysis and exploration. Continue with :ref:`analyse_hic` for further details.

If you want to explore individual matrix generation pipeline steps, continue to :ref:`bloom-modular`.
